...
export default async function CommunitySubchannelPage({ params }: { params: Promise<{ slug: string; subchannelSlug: string }> }) {
  ...
  const subchannel = channel?.subchannels.find((item) => item.slug === subchannelSlug);
  ...
  return <AppShell user={user}><main className={`communityLayout${subchannel.type === 'CHAT' ? ' communityChatLayout' : subchannel.type === 'CHECKLIST' ? ' communityChecklistLayout' : ''}`}><CommunityFeed channels={channels} initialPosts={posts.items} initialTotal={posts.meta.total} activeChannelSlug={slug} activeSubchannelSlug={subchannelSlug} currentUserId={user.id} currentUserName={user.fullName} canModerate={can(user, 'discussions.moderate')} />{subchannel.type !== 'CHECKLIST' ? <CommunityRail events={events} announcements={announcements} /> : null}</main></AppShell>;
}
