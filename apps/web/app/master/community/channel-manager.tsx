...
export function ChannelManager({ initialChannels }: { initialChannels: ManagedChannel[] }) {
  ...
  const [channelDraft, setChannelDraft] = useState({ name: '', description: '' });
  ...
  function openEditChannel(group: ManagedChannel) {
    setEditingChannel(group.id); setChannelDraft({ name: group.name, description: group.description ?? '' });
  }

  function updateChannel(group: ManagedChannel) {
    if (channelDraft.name.trim().length < 2) return;
    startTransition(async () => { try {
      replace(group.id, unwrap<ManagedChannel>(await browserClient().PATCH('/api/v1/admin/community/channels/{id}', {
        params: { path: { id: group.id } }, body: { name: channelDraft.name.trim(), description: channelDraft.description.trim() || undefined },
      })));
      setEditingChannel(null); setMessage('Channel berhasil diperbarui.');
  ...
  async function deleteChannelPermanently(group: ManagedChannel) {
    const confirmation = await notifier.prompt('Hapus channel permanen', { label: `Ketik ${group.name} untuk mengonfirmasi`, minLength: group.name.length, confirmLabel: 'Hapus permanen' });
    if (confirmation !== group.name) { if (confirmation !== null) notifier.error('Nama channel tidak cocok.'); return; }
    startTransition(async () => { try {
      await browserClient().DELETE('/api/v1/admin/community/channels/{id}/permanent', { params: { path: { id: group.id } } });
      setChannels((items) => items.filter((item) => item.id !== group.id)); setMessage('Channel dihapus permanen.');
    } catch (error) { setMessage(error instanceof Error ? error.message : 'Channel gagal dihapus permanen.'); } });
  }
  ...
  return <div className="channelManager">
    <section className="channelAdminList"><div className="channelManagerHead"><div><span className="eyebrow">KOMUNITAS</span><h2>Channel dan sub-channel</h2><p className="communityMuted">Kelompokkan ruang chat, postingan, dan pengumuman seperti struktur sidebar.</p></div><div><span>{active.length} channel</span><button className="btn" type="button" aria-expanded={showCreate} onClick={() => setShowCreate(true)}><Plus size={16} />Tambah channel</button></div></div>{message ? <p role="status" className="communityMessage">{message}</p> : null}
      ...
      {archived.length ? <div className="channelArchive"><div className="channelListHeading"><h2>Channel terarsip</h2><span>{archived.length}</span></div>{archived.map((group) => <div className="card channelAdminItem channelArchived" key={group.id}><span className="channelHash">#</span><div><strong>{group.name}</strong><small>Seluruh isinya tersembunyi</small></div><ActionMenu><button type="button" onClick={() => restoreChannel(group)}>Pulihkan channel</button><button className="btnDanger" type="button" onClick={() => void deleteChannelPermanently(group)}>Hapus permanen</button></ActionMenu></div>)}</div> : null}
    </section>
    {showCreate ? <Modal title="Tambah channel" className="wideFormModal" description="Buat Channel bersama sub-channel pertamanya." busy={pending} onClose={() => setShowCreate(false)}><div className="channelForm adminModalForm"><div className="field"><label htmlFor="channel-name">Nama channel</label><input id="channel-name" value={name} onChange={(event) => setName(event.target.value)} placeholder="Contoh: Selamat datang" /></div><div className="field"><label htmlFor="channel-desc">Keterangan channel</label><textarea id="channel-desc" value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Jelaskan isi kelompok ini" /></div><div className="channelFormDivider"><strong>Sub-channel pertama</strong><small>Pilih bentuk ruang yang sesuai dengan isi dan cara interaksinya.</small></div><SubchannelFields id="first" value={firstSub} onChange={setFirstSub} /><div className="channelAdminActions"><button className="btn secondary" type="button" disabled={pending} onClick={() => setShowCreate(false)}>Batal</button><button className="btn" type="button" disabled={pending || name.trim().length < 2 || firstSub.name.trim().length < 2} onClick={createChannel}>Buat channel</button></div></div></Modal> : null}
    {addingGroup ? <Modal className="wideFormModal" title={`Tambah sub-channel ke ${addingGroup.name}`} description="Pilih bentuk ruang dan aturan interaksinya." busy={pending} onClose={() => setAddingTo(null)}><div className="channelForm adminModalForm"><SubchannelFields id={addingGroup.id} value={draft} onChange={setDraft} /><div className="channelAdminActions"><button className="btn secondary" type="button" disabled={pending} onClick={() => setAddingTo(null)}>Batal</button><button className="btn" type="button" disabled={pending || draft.name.trim().length < 2} onClick={() => createSubchannel(addingGroup)}>Tambah sub-channel</button></div></div></Modal> : null}
    {editingGroup && editingSub ? <Modal className="wideFormModal" title={`Edit ${editingSub.name}`} description="Ubah kategori dan aturan interaksi sub-channel." busy={pending} onClose={() => setEditing(null)}><div className="channelForm adminModalForm"><SubchannelFields id={`edit-${editingSub.id}`} value={editDraft} onChange={setEditDraft} /><div className="channelAdminActions"><button className="btn secondary" type="button" disabled={pending} onClick={() => setEditing(null)}>Batal</button><button className="btn" type="button" disabled={pending || editDraft.name.trim().length < 2} onClick={() => updateSubchannel(editingGroup, editingSub)}>Simpan perubahan</button></div></div></Modal> : null}
    {editingChannel ? (() => { const group = channels.find((item) => item.id === editingChannel); return group ? <Modal className="wideFormModal" title="Edit channel" description="Ubah nama dan keterangan yang dilihat pengguna." busy={pending} onClose={() => setEditingChannel(null)}><div className="channelForm adminModalForm"><div className="field"><label htmlFor="edit-channel-name">Nama channel</label><input id="edit-channel-name" value={channelDraft.name} onChange={(event) => setChannelDraft((value) => ({ ...value, name: event.target.value }))} /></div><div className="field"><label htmlFor="edit-channel-description">Keterangan channel</label><textarea id="edit-channel-description" value={channelDraft.description} onChange={(event) => setChannelDraft((value) => ({ ...value, description: event.target.value }))} placeholder="Jelaskan isi channel ini" /></div><div className="channelAdminActions"><button className="btn secondary" type="button" disabled={pending} onClick={() => setEditingChannel(null)}>Batal</button><button className="btn" type="button" disabled={pending || channelDraft.name.trim().length < 2} onClick={() => updateChannel(group)}>Simpan perubahan</button></div></div></Modal> : null; })() : null}
  </div>;
}
