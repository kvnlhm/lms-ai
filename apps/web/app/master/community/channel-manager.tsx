...
type SubDraft = { name: string; description: string; type: CommunityChannelType; isReadOnly: boolean; showInSidebar: boolean };
const EMPTY_SUB: SubDraft = { name: '', description: '', type: 'CHAT', isReadOnly: false, showInSidebar: true };
...
export function ChannelManager({ initialChannels }: { initialChannels: ManagedChannel[] }) {
    ...
    startTransition(async () => {
      try {
        const created = unwrap<ManagedChannel>(await browserClient().POST('/api/v1/admin/community/channels', { body: {
          ...
          subchannelType: firstSub.type, isReadOnly: firstSub.isReadOnly, showInSidebar: firstSub.showInSidebar,
        } }));
  ...
  function openEdit(group: ManagedChannel, sub: CommunitySubchannel) {
    ...
    setEditDraft({ name: sub.name, description: sub.description ?? '', type: sub.type, isReadOnly: sub.isReadOnly, showInSidebar: sub.showInSidebar });
  }
