...
/**
 * Seluruh tes lama berbicara tentang pelajar yang sudah membayar — itulah
 * satu-satunya jenis akun yang ada ketika mereka ditulis. Gerbang keanggotaan
 * diuji terpisah di bawah.
 */
const BERBAYAR = { anggotaBerbayar: jest.fn().mockResolvedValue(true) };
const GRATIS = { anggotaBerbayar: jest.fn().mockResolvedValue(false) };
...
describe('CommunityService hierarchy invariants', () => {
  test('sidebar hanya meminta Channel dan Sub-channel yang dipilih Master', async () => {
    ...
    const service = new CommunityService({ communityChannelGroup: { findMany } } as never, {} as never, {} as never, BERBAYAR as never);
  ...
  test('sub-channel aktif terakhir tidak dapat diarsipkan', async () => {
    ...
    } as never, { record: jest.fn() } as never, {} as never, BERBAYAR as never);
  ...
  test('sub-channel pengumuman selalu dibuat baca-saja', async () => {
    ...
    } as never, {} as never, {} as never, BERBAYAR as never);
  ...
  test('post pengumuman tidak menerima komentar maupun reaksi', async () => {
    ...
    const service = new CommunityService(prisma as never, {} as never, {} as never, BERBAYAR as never);
  ...
  test('setiap item checklist menyimpan judul dan kontennya secara terpisah', async () => {
    ...
    const service = new CommunityService(prisma as never, {} as never, { bind: jest.fn() } as never, BERBAYAR as never);
  ...
  test('lampiran diikat di dalam transaksi yang sama dengan pembuatan tulisannya', async () => {
    ...
    const service = new CommunityService(prisma as never, {} as never, { bind } as never, BERBAYAR as never);
  ...
  test('Master dapat menyunting item checklist meskipun dibuat pengguna lain', async () => {
    ...
    } as never, { record } as never, {} as never, BERBAYAR as never);
...
describe('jajak pendapat', () => {
  function layanan(prismaExtra: Record<string, unknown> = {}) {
    ...
    return { prisma, service: new CommunityService(prisma as never, {} as never, { bind: jest.fn() } as never, BERBAYAR as never) };
  }
...
describe('CommunityService gerbang keanggotaan', () => {
  /**
   * Akun gratis membaca komunitas tetapi tidak menulis di dalamnya (ADR-032).
   *
   * Diuji lewat setiap jalur tulis, bukan hanya satu: gerbang yang terpasang di
   * empat dari lima jalur bukan gerbang, dan yang kelima itulah yang akan
   * ditemukan orang.
   */
  function gratis() {
    const prisma = {} as never;
    return new CommunityService(prisma, {} as never, {} as never, GRATIS as never);
  }

  const jalurTulis: Array<[string, (service: CommunityService) => Promise<unknown>]> = [
    ['membuat tulisan', (s) => s.createPost('gratis-1', 'sub-1', 'Isi', false)],
    ['mengomentari', (s) => s.addComment('gratis-1', 'post-1', 'Balasan')],
    ['bereaksi', (s) => s.toggleReaction('gratis-1', 'post-1')],
    ['memilih pada polling', (s) => s.votePoll('gratis-1', 'post-1', 'opt-1')],
    ['mencentang checklist', (s) => s.setChecklistCompleted('gratis-1', 'post-1', true)],
  ];

  for (const [nama, jalankan] of jalurTulis) {
    test(`${nama} ditolak 402, bukan 403`, async () => {
      await expect(jalankan(gratis())).rejects.toMatchObject({
  ...
  test('moderator tidak ditanyai keanggotaan sama sekali', async () => {
    // Kewenangan Master berasal dari permission, dan akun Master tidak selalu
    // punya pesanan berbayar atas namanya.
    const keanggotaan = { anggotaBerbayar: jest.fn().mockResolvedValue(false) };
    const prisma = {
      communityChannel: { findFirst: jest.fn().mockResolvedValue(null) },
    } as never;
    const service = new CommunityService(prisma, {} as never, {} as never, keanggotaan as never);

    // Ditolak karena sub-channel-nya tidak ada, bukan karena keanggotaan.
    await expect(service.createPost('master-1', 'sub-hilang', 'Isi', true)).rejects.toMatchObject({
      status: 404,
    });
    expect(keanggotaan.anggotaBerbayar).not.toHaveBeenCalled();
  });
});
